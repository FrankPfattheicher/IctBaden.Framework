...
public partial class Profile
{
    ...
    public Profile(string profileName, Encoding desiredEncoding, string? commentLines)
    {
        ...
        if (!string.IsNullOrEmpty(profileName) && File.Exists(profileName))
        {
            ...
            ReadOnly = (File.GetAttributes(profileName) & FileAttributes.ReadOnly) != FileAttributes.None;
            Load();
        }
        else
        {
    ...
    public static bool NeedsUnicode(string text)
    {
        ...
        return !string.Equals(text, ascii, StringComparison.OrdinalIgnoreCase);
    }

    #region LoadAndSave

    private static readonly Regex SectionFmt = RegexSectionFmt();
    private static readonly Regex KeyFmt = RegexKeyFmt();
    ...
    private void LoadContent(TextReader content)
    {
        ...
        while (true)
        {
            ...
            if (string.IsNullOrEmpty(line))
                continue;
            if ((CommentLines != null) && line.StartsWith(CommentLines, StringComparison.InvariantCultureIgnoreCase))
                continue;
    ...
    public bool Load()
    {
        ...
        try
        {
            ...
            using (var fileData = new StreamReader(FileName, FileEncoding, detectEncodingFromByteOrderMarks: true))
            {
...
#pragma warning disable MA0023
    ...
    private static partial Regex RegexSectionFmt();
    [GeneratedRegex("^([^=]+)=(.*)$", RegexOptions.Compiled)]
    private static partial Regex RegexKeyFmt();
